...
const CharacterGrid = ({ onCharacterClick, activeSection, onSessionOpen }) => {
  ...
  // Show create button in Discover section
  const showCreateButton = activeSection === 'Discover';

  return (
    <Box className={classes.section}>
      {showSuccess && (
        <Alert severity="success" className={classes.successAlert}>
          🎉 Character created successfully! It should appear in the list shortly.
        </Alert>
      )}
      ...
      {paginationLoading ? (
        ...
        <>
          <Box className={classes.characterBoxContainer}>
            {/* Create Character Button - only show in Discover section */}
            {showCreateButton && (
