...
// src/services/api.js - Updated API Service with new endpoints
import axios from 'axios';
...
class ApiService {
  ...
  // NEW: Character search endpoint
  async searchCharacters(searchTerm, page = 1, perPage = 20) {
    try {
  ...
  // Character endpoints
  async getCharacters(page = 1, perPage = 50) {
    try {
      const response = await this.client.get('/characters', {
  ...
  async getSupportedLanguages() {
    try {
      ...
      throw this.handleError(error, 'Failed to load languages');
    }
  }

  // Enhanced chat endpoint with language support
  async sendMessage(characterName, userInput, options = {}) {
    try {
      const requestData = {
        character_name: characterName,
        user_input: userInput,
        ...
        ...options.creativitySettings,
      };

      const response = await this.client.post('/chat', requestData);
      
      // Ensure chat_history is sorted by timestamp
      if (response.data.chat_history && Array.isArray(response.data.chat_history)) {
        response.data.chat_history = this.sortMessagesByTimestamp(response.data.chat_history);
      }
  ...
  // Character creation with enhanced options
  async createCharacter(characterData) {
    try {
      const response = await this.client.post('/create_character', characterData);
      return response.data;
